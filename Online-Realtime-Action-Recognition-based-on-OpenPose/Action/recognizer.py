...
#                 # id = int(d[4])
#                 try:
#                     # xcenter is the joint point of all humans in a frame of image(Neck) x coordinate value
#                     # By calculating the distance between track_box and human xcenter，Match ID
#                     tmp = np.array([abs(i - (xmax + xmin) / 2.) for i in xcenter])
#                     j = np.argmin(tmp)
#                 except:
#                     # If there is no human in the current frame，Default j=0(invalid)
#                     j = 0
#
